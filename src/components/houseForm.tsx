...
export default function HouseForm({ house }: IProps) {
  ...
  const handleUpdate = async (currentHouse: IHouse, data: IFormData) => {
    // this is the default - if the user didn't change the image,
    // the value is the same
    let image = currentHouse.image;
    // this next block is if the user has updated the image
    if (data.image[0]) {
      const { data: signatureData } = await createSignature();
      if (signatureData) {
        const { signature, timestamp } = signatureData.createImageSignature;
      ...
      {address && (
        <>
          ...
          <div className="mt-4">
            <label
              htmlFor="image"
              className="p-4 border-dashed border-4 block cursor-pointer border-gray-600"
            >
